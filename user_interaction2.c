...
void save_traversal(struct dictNode *node, FILE *fn){
	...
	//Recursion of the left subtree
	save_traversal(node->leftChild, fn);

	//Save word and definition in one CSV line. Line is word,def\n
	char line[10000];
	strcat(line, node->word);
	strcat(line, ",");
	strcat(line, node->def);
	strcat(line, "\n\0");
	fputs(line, fn);

	//Recursion of the right subtree
	save_traversal(node->rightChild, fn);
}

void deleteEverything(struct linkedDict **dict){
	for (int i = 0; i < 26; i++){
		while (dict[i]->root != NULL){
			printf("here\n");
			deleteNode(dict[i]->root, dict[i]);
		}
		free(dict[i]);
	}
	free(dict);
